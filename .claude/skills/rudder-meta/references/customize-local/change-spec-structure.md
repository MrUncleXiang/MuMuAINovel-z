...
When the user wants to change the engineering conventions AI follows, add new spec layers, or adjust monorepo package mapping, edit `.rudder/spec/` and `.rudder/config.yaml`.

## Read These Files First

1. `.rudder/config.yaml`
2. `.rudder/spec/`
3. `.rudder/workflow.md` Phase 1.3 and Phase 3.3
...
| Need | Edit location |
| --- | --- |
| Add backend/frontend/docs/test spec layer | `.rudder/spec/<layer>/` or `.rudder/spec/<package>/<layer>/` |
| Add shared thinking guides | `.rudder/spec/guides/` |
| Adjust monorepo packages | `packages` in `.rudder/config.yaml` |
| Change default package | `default_package` in `.rudder/config.yaml` |
| Control spec scanning scope | `spec_scope` in `.rudder/config.yaml` |
...
```bash
python3 ./.rudder/scripts/task.py add-context <task> implement ".rudder/spec/webapp/security/index.md" "Security conventions"
python3 ./.rudder/scripts/task.py add-context <task> check ".rudder/spec/webapp/security/index.md" "Security review rules"
```

## Change Monorepo Packages

Example `.rudder/config.yaml`:
