...
Hooks are the automation layer that connects a platform to Rudder. When the user wants to change "when context is injected," "how shell commands inherit a session," or "which files are read before an agent starts," hooks are usually the edit point.
...
3. `.rudder/scripts/common/active_task.py`
4. `.rudder/scripts/common/session_context.py`
5. `.rudder/workflow.md`

## Common Hook Types

| Hook | Purpose |
| --- | --- |
| session-start | Injects a Rudder overview when a session starts, clears, or compacts. |
...
3. Read the hook script and identify inputs, outputs, and called `.rudder/scripts/`.
4. Modify hook behavior.
5. If the hook depends on workflow content, synchronize `.rudder/workflow.md`.
...
If the hook ultimately calls `.rudder/scripts/get_context.py` or `session_context.py`, editing the local script is usually more robust than hard-coding content in the hook.
...
- Hooks should read project-local `.rudder/`; they should not depend on Rudder upstream source paths.
