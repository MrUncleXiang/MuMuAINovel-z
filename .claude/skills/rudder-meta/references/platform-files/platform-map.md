...
This page lists common Rudder file locations in a user project by platform. Whether a platform directory exists in an actual project depends on which `rudder init --<platform>` commands the user ran.
...
| Pi Agent | `--pi` | `.pi/` | `.pi/skills/` | `.pi/agents/` | `.pi/extensions/rudder/` + `.pi/settings.json` |

## Capability Groups

### Rudder Sub-Agent Support

These platforms usually have `rudder-research`, `rudder-implement`, and `rudder-check` files:
...
When changing behavior, inspect workflows and skills first. Do not assume Rudder sub-agents exist.
...
4. User wants project rules: prefer `.rudder/spec/` or a project-local skill.
5. User wants Rudder behavior: edit `.rudder/workflow.md` plus platform hooks/agents/skills/commands.
