...
```
Development Flow:
  Learn something → /rudder:update-spec → Knowledge captured
       ↑                                  ↓
  /rudder:break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `/rudder:break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `/rudder:update-spec` - Actually makes the updates
- `/rudder:finish-work` - Reminds you to check if specs need updates
