...
const RUDDER_AGENT_JSONL: Record<string, string> = {
  "rudder-implement": "implement.jsonl",
  implement: "implement.jsonl",
  "rudder-check": "check.jsonl",
  check: "check.jsonl",
};
...
function findProjectRoot(startDir: string): string {
  ...
  while (true) {
    if (
      existsSync(join(current, ".rudder")) ||
...
function resolvePiInvocation(): PiInvocation {
  const envCli = stringValue(process.env.RUDDER_PI_CLI_JS);
  if (envCli) {
    const cliJs = resolve(envCli);
    if (!isExistingFile(cliJs)) {
      throw new Error(`RUDDER_PI_CLI_JS points to a missing file: ${cliJs}`);
    }
...
function normalizeTaskRef(raw: string): string | null {
  ...
  if (normalized.startsWith("tasks/")) normalized = `.rudder/${normalized}`;
  return normalized;
}

function taskRefToDir(projectRoot: string, taskRef: string): string {
  if (taskRef.startsWith("/")) return taskRef;
  if (taskRef.startsWith(".rudder/")) return join(projectRoot, taskRef);
  return join(projectRoot, ".rudder", "tasks", taskRef);
}
...
function activeRuntimeContextKeys(projectRoot: string): string[] {
  const sessionsDir = join(projectRoot, ".rudder", ".runtime", "sessions");
  try {
...
function buildRudderContext(
  ...
  if (!taskDir) {
    return "No active Rudder task found. Read .rudder/ before proceeding.";
  }

  const prd = readText(join(taskDir, "prd.md"));
  const info = readText(join(taskDir, "info.md"));
  const jsonlName = RUDDER_AGENT_JSONL[agent] ?? "";
...
// We therefore parse `.rudder/workflow.md` `[workflow-state:STATUS]...
...
function loadWorkflowBreadcrumbs(projectRoot: string): Record<string, string> {
  const workflow = readText(join(projectRoot, ".rudder", "workflow.md"));
  if (!workflow) return {};
...
//
// Spawns `python3 .rudder/scripts/get_context.py` (the same script other
...
function pythonExecutable(): string {
  const override = stringValue(process.env.RUDDER_PYTHON);
...
function buildSessionOverview(
  ...
  const script = join(projectRoot, ".rudder", "scripts", "get_context.py");
...
function readAgentDefinition(
  ...
  const normalized = agent.startsWith("rudder-") ? agent : `rudder-${agent}`;
...
function commandStartsWithRudderContext(command: string): boolean {
  const trimmed = command.trimStart();
  return (
    /^export\s+RUDDER_CONTEXT_ID=/.test(trimmed) ||
    /^RUDDER_CONTEXT_ID=/.test(trimmed) ||
    /^env\s+.*\bRUDDER_CONTEXT_ID=/.test(trimmed)
  );
}
...
function injectRudderContextIntoBash(
  event: unknown,
  contextKey: string,
): boolean {
  ...
  if (commandStartsWithRudderContext(rawCommand)) {
    return false;
  }
...
function buildSubagentPrompt(
  ...
  const normalized =
    agentName ?? normalizeAgentName(input.agent ?? "rudder-implement");
  const definition =
...
async function runSubagent(
  ...
  const agentName = normalizeAgentName(input.agent ?? "rudder-implement");
...
export default function rudderExtension(pi: {
  ...
  pi.registerTool?.({
    name: "subagent",
    label: "Subagent",
    description: "Run a Rudder project sub-agent with active task context.",
    ...
    parameters: {
      ...
      properties: {
        agent: {
          type: "string",
          description:
            "Agent name, such as rudder-implement or rudder-check.",
        },
        prompt: {
          type: "string",
    ...
    execute: async (
      ...
      return {
        ...
        details: {
          agent: input.agent ?? "rudder-implement",
  ...
  pi.on?.("session_start", (event, ctx) => {
    ...
    ctx?.ui?.notify?.(
      "Rudder project context is available. Use /rudder-continue to resume the current task.",
      "info",
    );
  });
