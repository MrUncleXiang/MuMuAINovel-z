...
Skills and commands are textual entry points for user interaction with Rudder. Different platforms use different names, but their core purpose is the same: tell the AI how to enter the Rudder flow when the user expresses a certain intent.
...
Rudder workflow skills usually share one semantic set: brainstorm, before-dev, check, update-spec, break-loop. Multi-file built-in skills such as `rudder-meta` use layered references.
...
They should not store task state; task state belongs in `.rudder/tasks/` and `.rudder/.runtime/`.
...
1. **Keep entry files short; references carry long content**. This matters especially for multi-file skills like `rudder-meta`.
...
4. **Put project-specific capabilities in local skills**. Do not put team-private flows into public `rudder-meta`.

If the user only wants local AI to know one more project rule, usually create a project-local skill or update `.rudder/spec/` instead of changing a Rudder built-in workflow skill.
