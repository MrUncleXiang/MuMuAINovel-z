...
2. `.rudder/scripts/get_context.py`
3. `.rudder/scripts/common/session_context.py`
4. `.rudder/scripts/common/task_context.py`
5. `.rudder/scripts/common/active_task.py`
...
| Source | Purpose |
| --- | --- |
| `.rudder/workflow.md` | Workflow and next-action hints. |
| `.rudder/tasks/<task>/prd.md` | Current task requirements. |
| `.rudder/tasks/<task>/implement.jsonl` | Spec/research to read before implementation. |
| `.rudder/tasks/<task>/check.jsonl` | Spec/research to read during checking. |
| `.rudder/spec/` | Project specs. |
| `.rudder/workspace/` | Session records. |
...
| Change hints on each user input | `[workflow-state:STATUS]` block in `.rudder/workflow.md`. The `inject-workflow-state` hook is parser-only and reads the block verbatim. |
...
```jsonl
{"file": ".rudder/spec/backend/index.md", "reason": "Backend conventions"}
{"file": ".rudder/tasks/04-28-x/research/api.md", "reason": "API research"}
```
...
- agent pull: edit the read steps in the corresponding `rudder-implement` / `rudder-check` agent file.
...
python3 ./.rudder/scripts/task.py list-context <task>
