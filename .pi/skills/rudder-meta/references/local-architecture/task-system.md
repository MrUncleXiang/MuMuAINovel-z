...
The Rudder task system is stored entirely under `.rudder/tasks/` in the user project. Each task is a directory containing requirements, context, research, state, and relationship information.
...
The user sees a "current task," but Rudder stores active task state per session.

```text
.rudder/.runtime/sessions/<context-key>.json
```
...
```jsonl
{"file": ".rudder/spec/cli/backend/index.md", "reason": "Backend conventions"}
{"file": ".rudder/tasks/04-28-example/research/api.md", "reason": "API research"}
```
...
python3 ./.rudder/scripts/task.py add-context <task> implement <file> <reason>
python3 ./.rudder/scripts/task.py validate <task>
python3 ./.rudder/scripts/task.py finish
python3 ./.rudder/scripts/task.py archive <task>
```
...
| Need | Edit location |
| --- | --- |
| Change the default task template | `.rudder/scripts/common/task_store.py` and task creation instructions. |
| Change status semantics | `.rudder/workflow.md`, workflow-state hook logic, and task usage conventions. |
| Add task lifecycle actions | `hooks.after_*` in `.rudder/config.yaml`. |
| Change context rules | Phase 1.3 in `.rudder/workflow.md` and related platform agent/hook instructions. |
| Change archive policy | `.rudder/scripts/common/task_store.py` / `task_utils.py`. |

These are local files in the user project. Do not default to editing Rudder CLI source code unless the user wants to contribute upstream.
