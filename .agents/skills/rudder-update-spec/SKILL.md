---
name: rudder-update-spec
description: "Captures executable contracts and coding conventions into .rudder/spec/ documents. Use when learning something valuable from debugging, implementing, or discussion that should be preserved for future sessions."
---
...
```
Development Flow:
  Learn something → `update-spec` (Rudder command) → Knowledge captured
       ↑                                  ↓
  `break-loop` (Rudder command) ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- ``break-loop` (Rudder command)` - Analyzes bugs deeply, often reveals spec updates needed
- ``update-spec` (Rudder command)` - Actually makes the updates
- ``finish-work` (Rudder command)` - Reminds you to check if specs need updates
