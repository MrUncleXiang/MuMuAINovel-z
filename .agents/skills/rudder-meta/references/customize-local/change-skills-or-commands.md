...
4. Whether project rules already exist in `.rudder/spec/`
...
| Team project conventions | Prefer `.rudder/spec/` or a project-local skill. |
| Change Rudder flow semantics | Synchronize `.rudder/workflow.md`. |
...
- Which `.rudder/` files to read.
- Which scripts to run.
- How to report after completion.

If a command only repeats workflow rules, prefer making it reference/read `.rudder/workflow.md` instead of maintaining a second copy of the flow.
...
```text
.claude/skills/project-rudder-local/
└── SKILL.md
```
...
- Do not hide long-term engineering conventions inside a command; write them to `.rudder/spec/`.
