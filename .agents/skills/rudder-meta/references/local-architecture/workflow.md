...
`.rudder/workflow.md` is the Rudder workflow source of truth inside the user project. An AI does not need Rudder source code to understand how the current project should move tasks forward; this file is enough.

## File Responsibilities

`.rudder/workflow.md` has three responsibilities:
...
- Platforms with sub-agent support: dispatch `rudder-implement` by default for implementation and `rudder-check` for checking.
- Platforms without sub-agent support: the main session reads skills such as `rudder-before-dev`, then executes directly.
...
After editing, make the AI reread `.rudder/workflow.md`; do not assume the flow from the old conversation is still valid.
...
- skills, such as `rudder-brainstorm` and `rudder-check`.
