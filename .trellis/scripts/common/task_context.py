...
def _validate_jsonl(jsonl_file: Path, repo_root: Path, task_dir: Path | None = None) -> int:
    ...
    if not jsonl_file.is_file():
        msg = f"{file_name}: not found (skipped)"
    ...
    for line in jsonl_file.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            msg = f"{file_name}:{line_num}: Invalid JSON"
            print(f"  {colored(msg, Colors.RED)}")
            errors += 1
            continue
        ...
        if entry_type == "directory":
            if not full_path.is_dir():
                msg = f"{file_name}:{line_num}: Directory not found: {file_path}"
                print(f"  {colored(msg, Colors.RED)}")
                errors += 1
            continue

        if not full_path.is_file():
            msg = f"{file_name}:{line_num}: File not found: {file_path}"
            print(f"  {colored(msg, Colors.RED)}")
            errors += 1
            continue
        ...
        if extension in _CODE_FILE_EXTENSIONS and not _is_exempt_from_code_file_warning(
            ...
            print(f"  {colored(warning_text, Colors.YELLOW)}")
        ...
        if max_file_bytes:
            ...
            if size > max_file_bytes:
                ...
                print(f"  {colored(size_warning, Colors.YELLOW)}")

    if errors == 0:
        msg = f"{file_name}: ✓ ({real_entries} entries)"
        print(f"  {colored(msg, Colors.GREEN)}")
    else:
        msg = f"{file_name}: ✗ ({errors} errors)"
        print(f"  {colored(msg, Colors.RED)}")
...
def cmd_list_context(args: argparse.Namespace) -> int:
    ...
    for jsonl_name in ["implement.jsonl", "check.jsonl"]:
        ...
        for line in jsonl_file.read_text(encoding="utf-8").splitlines():
            ...
            if entry_type == "directory":
                msg = f"{count}."
                print(f"  {colored(msg, Colors.GREEN)} [DIR] {file_path}")
            else:
