...
The user-facing concept is a single "active task". Rudder stores that pointer
per AI session/window under `.rudder/.runtime/sessions/`; without a stable
...
def _detect_platform(platform_input: dict[str, Any] | None, platform: str | None) -> str:
    ...
    if platform_input:
        for key in ("_rudder_platform", "rudder_platform", "platform", "source"):
...
def _lookup_env_context_key(platform_name: str | None) -> str | None:
    ...
    Hooks pass `RUDDER_CONTEXT_ID` to subprocesses they launch, but an AI-run
