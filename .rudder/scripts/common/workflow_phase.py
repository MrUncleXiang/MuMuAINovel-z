...
Extracts step-level content from .rudder/workflow.md and optionally filters
...
def get_phase_index() -> str:
    ...
    `## Customizing Rudder (for forks)` (the docs-for-forks footer).
    ...
    for i, line in enumerate(lines):
        ...
        if start is None and stripped == _PHASE_INDEX_HEADING:
            start = i
            continue
        if start is not None and stripped == "## Customizing Rudder (for forks)":
            end = i
            break
...
def resolve_effective_platform(platform: str, config: dict) -> str:
    ...
    or ``"codex-sub-agent"`` based on ``.rudder/config.yaml`` ``codex.dispatch_mode``.
