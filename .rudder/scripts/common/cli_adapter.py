...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "rudder-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'rudder-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'rudder', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/rudder-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Windsurf uses workflow directory: .windsurf/workflows/rudder-<name>.md
            Copilot uses prompt files: .github/prompts/<name>.prompt.md
            Pi uses prompt templates: .pi/prompts/rudder-<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/rudder/<name>.md
        """
        if self.platform == "pi":
            prompts_dir = self.get_config_dir(project_root) / "prompts"
            if not parts:
                return prompts_dir
            if len(parts) >= 2 and parts[0] == "rudder":
                filename = parts[-1]
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return prompts_dir / f"rudder-{filename}.md"
        ...
        if self.platform == "windsurf":
            ...
            if len(parts) >= 2 and parts[0] == "rudder":
                filename = parts[-1]
                return workflow_dir / f"rudder-{filename}"
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "rudder":
            # Convert rudder/<name>.md to rudder-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"rudder-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_rudder_command_path(self, name: str) -> str:
        """Get relative path to a rudder command file.
        ...
        Note:
            ...
            Codex: .agents/skills/rudder-<name>/SKILL.md
            Kiro: .kiro/skills/rudder-<name>/SKILL.md
            Gemini: .gemini/commands/rudder/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Windsurf: .windsurf/workflows/rudder-<name>.md
            Pi: .pi/prompts/rudder-<name>.md
            Others: .{platform}/commands/rudder/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/rudder-{name}.md"
        elif self.platform == "codex":
            # 0.5.0-beta.0 renamed all skill dirs to add the `rudder-` prefix
            ...
            return f".gemini/commands/rudder/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "windsurf":
            return f".windsurf/workflows/rudder-{name}.md"
        elif self.platform == "kilo":
            ...
            return f"{self.config_dir_name}/commands/rudder/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    Detection order:
    1. RUDDER_PLATFORM environment variable (if set)
    ...
    env_platform = os.environ.get("RUDDER_PLATFORM", "").lower()
    if env_platform in (
        "claude",
        "opencode",
        "cursor",
    ...
    # (.agents/skills/rudder-* dirs) and no explicit platform config dir.
    ...
    if agents_skills.is_dir() and not _has_other_platform_dir(
        ...
        try:
            for entry in agents_skills.iterdir():
                if entry.is_dir() and entry.name.startswith("rudder-"):
                    return "codex"
        except OSError:
            pass
